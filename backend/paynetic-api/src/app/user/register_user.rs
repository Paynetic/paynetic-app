...
pub async fn register_user(
    ...
    // Verify eth_address matches signature
    let eth_address = &dto.eth_address;
    if context
        .repo
        .user
    ...
    let signature = &dto.eth_address_signature;
    let message = format!("Register a Paynetic account with {}", eth_address);
