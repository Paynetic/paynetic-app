...
pub trait UserRepoTrait {
    ...
    async fn get_user_by_eth_address(&self, id: String) -> Result<UserEntity, DbError>;
    ...
    async fn find_user_by_eth_address(&self, email: String) -> Result<UserEntity, DbError>;
...
const USER_COLUMNS: &str = formatcp!(
    r#"{u}.id, {u}.name, {u}.description, {u}.link, {u}.location, {u}.email, {u}.password_hash, {u}.eth_address, {u}.created_at, {u}.updated_at,
...
fn map_user_entity_with_relations(row: PgRow) -> Result<UserEntity, sqlx::Error> {
    Ok(UserEntity {
        id: row.try_get("id")?,
        name: row.try_get("name")?,
        description: row.try_get("description")?,
...
impl UserRepoTrait for UserRepo {
    ...
    async fn create_user(
        ...
        Ok(sqlx::query(
            // language=PostgreSQL
            r#"
          INSERT INTO "users" (email, name, password_hash, eth_address, user_type, user_status)
          values ($1, $2, $3, $4, $5, $6)
          RETURNING id
      "#,
        )
        ...
        .map_err(|e| match e {
            ...
            sqlx::Error::Database(dbe) if dbe.constraint() == Some("users_eth_address_key") => {
                DbError::Unique("eth_address".into())
            }
    ...
    async fn update_user(&self, id: Uuid, params: UserUpdateParams) -> Result<UserEntity, DbError> {
        ...
        let (query, update_count) =
            append_comma(query, "eth_address", params.eth_address, update_count);
    ...
    async fn find_user_by_eth_address(&self, eth_address: String) -> Result<UserEntity, DbError> {
        let query = sqlx::query(formatcp!(
            r#"SELECT {}
                FROM users
                WHERE eth_address = $1
